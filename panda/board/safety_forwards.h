...
uint8_t safety_calculate_checksum(uint8_t *dat, uint32_t len) {
  uint8_t checksum = 0U;
...
void safety_can_set_checksum(CANPacket_t *packet) {
  packet->checksum = 0U;
  packet->checksum = safety_calculate_checksum((uint8_t *) packet, CANPACKET_HEAD_SIZE + GET_LEN(packet));
}
