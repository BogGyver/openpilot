...
class LatControl(object):
  ...
  def update(self, active, v_ego, angle_steers, angle_rate, angle_offset, steer_override, CP, VM, path_plan):
    ...
    if v_ego < 0.3 or not active:
      ...
      path_plan.cur_state[0].delta = math.radians(angle_steers - angle_offset) / CP.steerRatio
    else:
      # Interpolate desired angle between MPC updates
      self.angle_steers_des = np.interp(cur_time, path_plan.mpc_times, path_plan.mpc_angles)
      self.angle_steers_des_time = cur_time
      path_plan.cur_state[0].delta = math.radians(self.angle_steers_des - angle_offset) / CP.steerRatio
