...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, cruise_cancel):
    ...
    if (not enabled) and self.CP.openpilotLongitudinalControl and (frame %2 == 0):
      ...
      if self.CP.carFingerprint == CAR.AP2_MODELS:
        can_sends.append(self.tesla_can.create_ap2_long_control(350.0/3.6, tesla_accel_limits, tesla_jerk_limits, self.long_control_counter))
      if self.CP.carFingerprint == CAR.AP1_MODELS:
        can_sends.append(self.tesla_can.create_ap1_long_control(350.0/3.6, tesla_accel_limits, tesla_jerk_limits, self.long_control_counter))
      self.long_control_counter = (self.long_control_counter + 1) % 8
