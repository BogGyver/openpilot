...
class ACCMode:
    ...
    FOLLOW = _Mode(
        label="follow", autoresume=False, state=ACCState.STANDBY, adaptive=True
    )
    AUTO = _Mode(label="auto", autoresume=True, state=ACCState.STANDBY, adaptive=True)
    PLAINCC = _Mode(
        label="cc", autoresume=False, state=ACCState.STANDBY, adaptive=False
    )
...
class ACCController:
    ...
    def update_stat(self, CS, enabled):
        ...
        if (
            ...
            ready = (
                CS.cstm_btns.get_button_status(ACCMode.BUTTON_NAME) > ACCState.OFF
                and (
                    enabled
                    or (
            ...
            if (
                ...
                if self.adaptive:
                    self.acc_speed_kph = max(
                        CS.v_ego_raw * CV.MS_TO_KPH, self.speed_limit_kph
                    )
                else:
        ...
        if CS.cstm_btns.get_button_status(ACCMode.BUTTON_NAME) in [
            ACCState.STANDBY,
            ACCState.NOT_READY,
        ]:
            if (
                (enabled or not self.adaptive)
                and CruiseState.is_enabled_or_standby(CS.pcm_acc_status)
                and CS.v_ego > self.MIN_CRUISE_SPEED_MS
            ):
                CS.cstm_btns.set_button_status(ACCMode.BUTTON_NAME, ACCState.STANDBY)
    ...
    def update_acc(
        ...
        if (not self.adaptive) and self.enable_adaptive_cruise:
            self.acc_speed_kph = (
                CS.v_cruise_actual
            )  # if not CS.imperial_speed_units else CS.v_cruise_actual * CV.MPH_TO_KPH
            return button_to_press

        # disengage if cruise is canceled
        if (
            (not self.enable_adaptive_cruise)
            and (CS.pcm_acc_status >= 2)
            and (CS.pcm_acc_status <= 4)
        ):
            button_to_press = CruiseButtons.CANCEL
        lead_1 = None
        # if enabled:
