class Blinker:
    ...
    def update_state(self, CS, frame):
        ...
        if CS.turn_signal_stalk_state > 0 and CS.prev_turn_signal_stalk_state == 0:
            ...
            CS.turn_signal_stalk_state == 0 and CS.prev_turn_signal_stalk_state > 0
            ...
            if frame - self.blinker_on_frame_start <= self.tap_duration_frames:
                ...
                if CS.tapBlinkerExtension > 0 and self.override_frame_end == 0:
                    ...
                    total_blinks = (
                        3 + CS.tapBlinkerExtension
                    )  # 3 blinks are minimum and controlled by the car
                    self.override_frame_end = (
                        self.blinker_on_frame_start
                        + blink_duration_frames * total_blinks
                    )
